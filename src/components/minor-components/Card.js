...
export const Card = ({ svg, ...rest }) => {
  ...
  let num = 0;
    ...
    return (
        <>
        {rest.isCategoryType  ?
            <div className="group border-2 bg-white relative flex flex-col shadow-md rounded-3xl " onClick={() => goToBrandPage(rest.title)}>
                <div className="flex flex-col py-2 px-4">
                    <h2 className="mt-1 px-1 text-lg text-primaryColor">
                ...
                <div className="aspect-w-1 hover:animate-up flex justify-center w-full aspect-h-1 p-3 overflow-hidden rounded-t  xl:aspect-w-7 xl:aspect-h-8">
                    <img onClick={() => { setIsOpen(true)}} className="h-24 w-24 object-cover cursor-pointer object-center" src={svg} alt='prod' />
                </div>
            </div>
        : null}
        {rest.isProductType ?
            <>
                <div className="group w-full border-2 relative flex flex-col shadow-lg rounded-3xl ">
                    {rest.item?.featureProduct ? 
                    ...
                    <div className="flex flex-col py-2 px-4">
                        <h2 className="mt-1 px-1 text-lg font-bold text-textColor truncate">
                            {rest.title.substring(0, 12)}
                        </h2>
                        <p className="mt-1 px-1 text-lg font-medium text-primaryColor">
                            ${rest.price}
                        </p>
                    </div>
                </div>
